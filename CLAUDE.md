...
- `case/<name>` branches are the test cases (one focused commit off master each); `expected/<name>/` on master holds both halves of that case's verification — `checks.grep` (AUTHORED from the case's intent, survives regeneration) and `reports/` (RECORDED argocdf output, wiped and rewritten by `--regenerate`). Never derive a checks pattern from generated output. Every source of every catalog entry needs a `repoURL` and either a `path` or a `chart` (an OCI artifact source takes `path: .` for its root, and a `chart` also needs a `targetRevision`): ArgoCD refuses anything else with `InvalidSpecError` and never renders it, argocdf refuses it too, and the review gate fails a report carrying `invalid Application spec` unless `checks.grep` declares it with a `must:` line naming that literal (a broad `must:# Error:` deliberately does NOT count) — so a fixture the controller would refuse cannot be pinned by accident, on a case branch the bootstrap never validates any more than on master.
